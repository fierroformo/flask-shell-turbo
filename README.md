...
## Description
Working with `flask shell` out of the box means manually importing every model and helper you need each time you open a session. **Flask Shell Plus** solves this by:
...
- **Adding a new CLI command**, `flask shell-plus`, with flags for a plain Python fallback and SQL query logging.
...
## Installation
1. Install flask-shell-plus

   ```bash
   pip install flask-shell-plus
   ```

## Integration into a Flask Project
Call `init_shell_plus(app, db)` right after your `db` is initialized — typically in your app factory, after `db.init_app(app)`.

```python
from flask import Flask
from your_project.extensions import db
from flask_shell_plus import init_shell_plus

def create_app():
    app = Flask(__name__)
    # ... your config ...
    db.init_app(app)
    # Register shell_plus after db is initialized
    init_shell_plus(app, db)
...
```bash
FLASK_APP=app flask shell-plus
```
...
Example:
```bash
flask shell-plus --print-sql
```

### Standard `flask shell` also benefits
Even without using `shell-plus`, the regular command is enriched automatically:

```bash
flask shell
```

Because `init_shell_plus` registers a `shell_context_processor`, all discovered models and utilities are available there too — `shell-plus` simply adds the nicer IPython experience, autoreload, and the extra CLI flags on top.
...
- **Works everywhere** — enhances both the new `shell-plus` command and the existing `flask shell`, so teammates who haven't adopted it yet still benefit.
